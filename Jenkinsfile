pipeline {
  ...
  stages {
    stage('build dev') {
      when { branch 'dev' }
      steps {
        sh 'docker build -t ezraweb/files:dev --build-arg VITE_hcaptchaSitekey="${VITE_hcaptchaSitekey}" --build-arg VITE_gatewayUrl="${VITE_gatewayUrl}" VITE_authUrl="${VITE_authUrl}" .'
      }
    }
    stage('build main') {
      when { branch 'main' }
